...
function ThemeToggle() {
  ...
  return (
    <button
      ...
      className="p-2 rounded text-slate-300 hover:text-white hover:bg-slate-700 transition-colors text-sm"
    ...
    <header className="relative bg-slate-900 text-white shadow-lg">
      ...
      <div className="w-full px-4 flex items-center justify-between h-14">
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Link href={`/?${buildParams()}`} className="font-bold text-base text-white hover:text-slate-200 transition-colors">
            ユビレジ
          </Link>
        </div>
        <div className="flex items-center gap-2">
          {!isDashboard && (
            <select
              value={currentA}
              onChange={e => { setNavigating(true); router.push(`${pathname}?${buildParams({ a: e.target.value })}`) }}
          ...
          <nav className="flex items-center gap-0.5">
            {APP_LINKS.map(link => (
              <Link key={link.href} href={`${link.href}?${buildParams()}`}
